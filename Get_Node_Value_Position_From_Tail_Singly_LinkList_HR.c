...
SinglyLinkedListNode* current; //First Pointer
current=malloc(sizeof(SinglyLinkedListNode));
current=head;

SinglyLinkedListNode* actual; //2nd Pointer
actual=malloc(sizeof(SinglyLinkedListNode));
actual=head;
...
count = count-1;
int actual_pos = count-positionFromTail;
int act_count=0;
while(act_count!=actual_pos){
    act_count++;
    actual=actual->next;
}
int pos_value;
pos_value = actual->data;
return(pos_value);
}
